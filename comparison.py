...
try:
    print("[Start] Sentence-BERT model load")
    SBERT_MODEL = SentenceTransformer('all-MiniLM-L6-v2')
    print("[Done] Sentence-BERT model load\n")
except Exception as e:
    print(f"Error: Failed to load Sentence-BERT. Error Message: {e}")
    SBERT_MODEL = None
...
def prepare_data_for_analysis(exp_file: str, methods_file: str) -> pd.DataFrame:
    try:
        ...
        print(f"   -> Error: Can't find '{exp_file}'.")
        return pd.DataFrame()
    except Exception as e:
        print(f"   -> Error: Failed to process '{exp_file}'. {e}")
        return pd.DataFrame()
    ...
    try:
        ...
        print(f"   -> Error: Can't find '{methods_file}'.")
        return pd.DataFrame()
    except Exception as e:
        print(f"   -> Error: Failed to process '{methods_file}'. {e}")
        return pd.DataFrame()
...
def main_analysis(exp_file: str, methods_file: str):
    ...
    output_details_file = 'analysis_details_multimetric.csv'
    ...
    ).sort_values(by=['Mean_F1_Score', 'Mean_Reasoning_Score'], ascending=[False, False])
    
    print(final_scores.to_string(float_format="%.4f"))
    
    output_summary_file = 'final_performance_summary_multimetric.csv'
    final_scores.to_csv(output_summary_file, encoding='utf-8-sig')
    print(f"\n -> Final output was saved to '{output_summary_file}'.")

if __name__ == "__main__":
    experiment_file_path = '/Users/sohyeon/hybridsystem/experiment_results.csv'
